...
architecture arch of controlador is 
	type estados is (INICIAL, LIGADO, PREPARA, CONTA, FIM, ESPERA, ERRAMENTACAO);
...
begin
	...
	proximo_estado <= INICIAL when (estado_atual = INICIAL and liga  = '0') else 
					  ...
					  CONTA   when (estado_atual = CONTA   and sinal = '1' and ov = '0') else 
					  FIM     when (estado_atual = CONTA   and sinal = '0' and ov = '0') else 
					  ESPERA  when (estado_atual = FIM) else 
					  ESPERA  when (estado_atual = ESPERA  and (sinal = '0' and liga = '1')) else 
					  
					  ERRAMENTACAO when ((estado_atual = CONTA and ov = '1')) else
						ERRAMENTACAO	 when ((estado_atual = ERRAMENTACAO)) else
					  estado_atual; 
						
	-- Saídas 
	--with estado_atual select zeraCont   <= '1' when ((PREPARA or ERRAMENTACAO) = '1'), '0' when others;
	with estado_atual select contaCont 	<= '1' when CONTA, 	 '0' when others;
	with estado_atual select pronto     <= '1' when FIM, 	 '0' when others;
	with estado_atual select error     <= '1' when ERRAMENTACAO, 	 '0' when others;
	zeraCont <= '1' when (estado_atual = PREPARA) else
					'1' when (estado_atual = ERRAMENTACAO ) else 
					'0';
					
	--error <= '1' when (estado_atual = ERRAMENTACAO ) else
	--			'0';
	...
	db_estado <= "0001" when (estado_atual = INICIAL) else
                 ...
                 "0110" when (estado_atual = ESPERA)  else
					  "0111" when (estado_atual = ERRAMENTACAO) else
                 "0000";
