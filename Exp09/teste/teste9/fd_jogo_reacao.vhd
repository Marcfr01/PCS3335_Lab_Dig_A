...
--
   ...
   --    );
	...
	--				 pontos_t ;
	-- Processo síncrono para garantir a contagem correta e evitar loops combinacionais
process(clock, reset)
begin
    if reset = '1' then
        pontos_t <= (others => '0'); -- Reset geral do sistema
    elsif rising_edge(clock) then
        if zerarPts = '1' then
            pontos_t <= (others => '0'); -- Comando da UC para zerar a pontuação
        elsif somarPts = '1' then
            pontos_t <= pontos_t + acertou_nota_perfeito; -- Soma apenas na borda de subida
        end if;
    end if;
end process;
    ...
    CONT_MS0 : contador
        ...
        port map (
            clock  => clock,
            clear  => zerarPts, --resetCont,
            enable => somarPts, --enableCont,
            Q      => q_ms0,
            RCO    => rco0
        );
    ...
    enable0 <= somarPts and rco0;
    enable1 <= somarPts and rco0 and rco1;
    enable2 <= somarPts and rco0 and rco1 and rco2;
